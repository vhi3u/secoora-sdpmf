...
for i = 1:length(gap_loc)
    %%%%% SH -- was just 200, now using 7*200 to be largest possible length, but it might not
    %%%%% check last one or two surfacings.  
    if gap_loc(i)+7*200 <= length(pres)
    ...
    %%%%% SH -- this code does not jive with comment above and it breaks in
    %%%%% some cases if gap_loc(i)+7*loc(i) is larger than the length of data vector.
