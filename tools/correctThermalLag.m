function [temp_inside, cond_outside, temp_corr, cond_corr] = correctThermalLag(varargin)
...
%  along with this program.  If not, see <http://www.gnu.org/licenses/>.
  ...
  % SMH -- unpin the first correction so not zero
  temp_correction(1) = temp_correction(2);
  cond_correction(1) = cond_correction(2);
  ...
  % SMH -- recast correction data for output 
  temp_corr = nan(size(timestamp));
  cond_corr = nan(size(timestamp));
  temp_corr(valid) = temp_correction;
  cond_corr(valid) = cond_correction;
